...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParIdentite() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a WHERE a.identite=:identite", Acteur.class).setParameter("identite", "Marion Cotillard");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParAnneeNaissance() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a WHERE YEAR(a.anniversaire)=:date", Acteur.class).setParameter("date", 1985);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles r WHERE r.nom=:name", Acteur.class).setParameter("name", "Harley Quinn");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.pays p JOIN f.realisateurs re WHERE re.identite=:nameRea AND f.annee BETWEEN 2010 AND 2020", Acteur.class).setParameter("nameRea", "Ridley Scott");
		List<Acteur> acteurs = query.getResultList();
